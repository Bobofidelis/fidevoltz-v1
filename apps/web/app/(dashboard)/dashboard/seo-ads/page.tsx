...
function StatCard({ title, value, subtitle, icon: Icon, accentColor = "blue" }: any) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Ads" value={analytics?.totalAds || 0} subtitle={`${analytics?.activeAds || 0} active`} icon={Target} accentColor="blue" />
          <StatCard title="Impressions" value={(analytics?.totalImpressions || 0).toLocaleString()} subtitle="Total ad views" icon={Eye} accentColor="green" />
          <StatCard title="Clicks" value={(analytics?.totalClicks || 0).toLocaleString()} subtitle={`${analytics?.ctr || 0}% CTR`} icon={MousePointerClick} accentColor="purple" />
          <StatCard title="Ad Revenue" value={`$${(analytics?.totalRevenue || 0).toFixed(2)}`} subtitle="Total earnings" icon={DollarSign} accentColor="amber" />
        </div>
      )}
          ...
          {seoLoading ? (
            ...
            <>
              <div className="grid md:grid-cols-3 gap-4">
                <StatCard title="SEO Score" value={`${seoData?.stats?.avgSeoScore || 0}/100`} subtitle="Average across all pages" icon={BarChart3} accentColor="blue" />
                <StatCard title="Mobile Score" value={`${seoData?.stats?.avgMobileScore || 0}/100`} subtitle="Mobile optimization" icon={TrendingUp} accentColor="green" />
                <StatCard title="Pages Tracked" value={seoData?.stats?.totalPages || 0} subtitle="Total pages monitored" icon={Activity} accentColor="purple" />
              </div>
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
